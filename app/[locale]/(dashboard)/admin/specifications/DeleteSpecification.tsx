...
const DeleteSpecification = ({ id }: Props) => {
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const response = await axios.delete(`/api/admin/specifications/${id}`);
  ...
  return (
    <>
      ...
      <Modal show={isOpen} size="xl" onClose={() => setIsOpen(false)}>
        <Modal.Header>
          <h2 className="text-xl font-semibold">Delete Specification</h2>
        </Modal.Header>
        <Modal.Body>
          <p className="mt-8">
